...
js.winiframe=function(tit, url, w1, h1){
	var mxw= 900;
	...
	js.tanbody('winiframe',tit,wi,410,{
		html:'<div style="height:'+hm+'px;border-bottom-left-radius:5px;border-bottom-right-radius:5px;overflow:hidden"><iframe src="" name="openinputiframe" width="100%" height="100%" frameborder="0"></iframe></div>',
		bbar:'none'
	});
	openinputiframe.location.href=url;
	return false;	
}
js.winiframeclose=function(){
...
js.tanbody=function(act,title,w,h,can1){
	...
	s+='<div id="'+mid+'" tanbody="rock" style="position:'+posta+';background-color:#ffffff;left:'+l+'px;width:'+w+'px;top:'+t+'px;z-index:'+this.tanbodyindex+';box-shadow:0px 0px 10px rgba(0,0,0,0.3);border-radius:5px">';
	s+='	<div class="'+can.titlecls+'" style="-moz-user-select:none;-webkit-user-select:none;user-select:none;background:#336699;border-radius:5px;border-bottom-left-radius:0;border-bottom-right-radius:0">';
	s+='		<table border="0"  width="100%" cellspacing="0" cellpadding="0"><tr>';
	s+='			<td height="40" style="font-size:16px; font-weight:bold;color:white; padding-left:8px" width="100%" onmousedown="js.move(\''+mid+'\')" id="'+act+'_title">'+title+'</td>';
	s+='			<td><div onmouseover="this.style.backgroundColor=\'#C64343\'" onmouseout="this.style.backgroundColor=\'\'" style="padding:0px 8px;height:40px;overflow:hidden;cursor:pointer;border-top-right-radius:5px" onclick="js.tanclose(\''+act+'\',\''+can.guanact+'\')"><div id="'+act+'_spancancel" style="height:16px;overflow:hidden;width:16px;background:url(/images/wclose.png);margin-top:10px"></div></div></td>';
	s+='		</tr></table>';
	s+='	</div>';
...
js.xpbody=function(act,type){
	...
	var W	= document.documentElement.scrollWidth+document.body.scrollLeft-10;
	var bs='<div id="xpbg_bodydds" xpbody="'+act+'" oncontextmenu="return false" style="position:absolute;display:none;width:'+W+'px;height:'+(H-5)+'px;filter:Alpha(opacity=30);opacity:0.3;left:5px;top:5px;background-color:#000000;z-index:1200"></div>';
...
js.alert = function(txt,tit,fun){
	if(txt=='none'){js.tanclose('confirm');return;}
...
js.msg = function(lx, txt,sj){
	...
	var s = '<div onclick="$(this).remove()" id="msgshowdivla" style="position:fixed;top:'+t+'px;z-index:9999;" align="center"><div style="padding:8px 20px;background:rgba(0,0,0,0.7);color:white;font-size:16px;">'+txt+'</div></div>';
	$('body').append(s);
...
js.ajax = function(url,da,fun,type,efun, tsar){
	...
	var ajaxcan={
		...
		success:function(ret){
			...
			js.debug(e);
			efun(e.responseText, 500);
			js.ajaxerror(e.responseText, 500);
		}
	};
...
//开发辅助
js.openkaifa=function(lx,path){
	var url = 'http://127.0.0.1/app/xinhu/?m=test&d=public&a=pingtal&ajaxbool=true&lx='+lx+'&file='+path+'&path=rockdoc&callback=?';

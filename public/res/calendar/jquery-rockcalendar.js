(function ($) {
	...
	function calendarclass(element, options){
		...
		this.init = function(){
			...
			s+='<tr>';
			for(var i=0;i<7;i++){
				s+='<td class="thtext" style="background-color:'+can.headerbgcolor+';border:'+bo1r+'px '+can.bordercolor+' solid" align="center">'+this.week[i]+'</td>';
			}
			s+='</tr>';
			var x	= 0,w = 100/7;
			var h   = can.height-26;
			for(var j=1;j<=6;j++){
				...
				for(var i=0;i<7;i++){
					x++;
					s+='<td align="'+can.align+'" height="'+(h/6)+'px" valign="'+can.valign+'" width="'+w+'%" style="border:'+bo1r+'px '+can.bordercolor+' solid" class="tdtext" id="calcontabc'+x+'_'+id+'" temp="'+x+'"></td>';
				}
				s+='</tr>';
			} 
			s+='</table></div>';

...
(function ($) {
	...
	function bootstable(element, options){
		...
		this._init	= function(){
			...
			if(!sas)sas='index:';
			sas 	= sas.split(':');
			if(can.url=='')can.url = js.getapiurl('docfile');
			if(can.cellurl=='')can.cellurl = js.getapiurl('docfile','savefields');
		...
		this._scrollTopla= function(o){
			...
			if(top<40){
				...
				if(!get('headerla_'+rand+'')){
					s = '<div style="position:fixed;left:'+(off.left)+'px;top:'+(off.top)+'px;width:100%;z-index:2;background:white" id="headerla_'+rand+'"></div>';
					obj.append(s);
				}
				var o1 = $('#headerla_'+rand+'');
				o1.show();
				...
				if(o1.html()==''){
					s = '';
					s+='<table class="table table-striped table-bordered table-hover" style="margin:0px" width="100%">';
					s+='<thead><tr><th width="40"></th>';
		...
		this._celleditla = function(o1){
			...
			s+='<div style="box-shadow:0px 0px 10px rgba(0,0,0,0.3);margin:0;margin-bottom:10px" class="well">';
			s+='	<div>&nbsp;<b>'+b.text+'</b>'+ets+'：&nbsp;<span id="msgteita_'+rand+'"></span></div>';
			...
			if(flx=='checkbox'){
				...
				s+='<div><textarea type="form-control" style="width:'+wss+'px;height:100px" '+attr+' id="inputedit_'+rand+'" class="input">'+v+'</textarea></div>';
				...
				s+='<div><input type="'+flx+'" style="width:'+wss+'px" id="inputedit_'+rand+'" '+attr+' class="form-control" value="'+v+'"></div>';
			}
			s+='	<div class="blank10"></div>';
			...
			s+='</div>';
			s+='<div align="center" style="font-size:20px;width:100%;left:0;bottom:0;height:16px;overflow:hidden;position:absolute"><i class="glyphicon glyphicon-triangle-bottom"></i></div>';
			s+='</div>';
			$('body').append(s);
		...
		this._editforcuschen = function(o1, a, farr){
			...
			$.ajax({
				data:this.signature(data, can.cellurl),type:'post',url:can.cellurl,dataType:'json',
		...
		this._loaddata = function(p, donbo){
			...
			h+=$('#tablefanye_'+rand+'').height()+10;
			s='<div id="modeshow_'+rand+'" style="filter:Alpha(opacity=20);opacity:0.2;height:'+h+'px;width:'+w+'px;overflow:hidden;z-index:3;position:absolute;left:'+l1+'px;line-height:'+h+'px;top:'+t1+'px;background:#000000;color:white" align="center"><img src="/images/mloading.gif"  align="absmiddle">&nbsp;加载中...</div>';
			$('body').append(s);
			can.beforeload();
			this.bool = true;
			$.ajax({
				url:url,type:can.method,data:das,dataType:'json',
				success:function(da){
		...
		this._caozuochengs=function(a,oj){
			a.renderer=function(v,d,oi){
				var s='<a role="button" oi="'+oi+'" oj="'+oj+'" temp="caozuomenu_'+rand+'">操作<i class="icon-angle-down"></i></a>';
				if(!d.id)s='&nbsp;';
				return s;
			};
			a.notexcel=true;
